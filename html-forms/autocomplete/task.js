class Autocomplete {
  ...
  getMatches( text ) {
    ...
    // Проходим по всем предложенным опциям
    for (let i = 0; i < this.input.options.length; i++) {

      // если введный в инпут текст ищется в одной из опции, то добавляем ее в массив
      if (this.input.options[i].innerHTML.includes(text)) {

        matches.push(
          {
            text: this.input.options[i].innerHTML,
            value: i
          }
        )  
